...
public class CycleWheelView extends ListView {
				...
				if (scrollState == SCROLL_STATE_IDLE) {
					...
					if (itemView != null) {
						...
						if (Math.abs(deltaY) < mItemHeight / 2) {
							...
							smoothScrollBy(getDistance(mItemHeight + deltaY),
									50);
						}
					}
				}
	...
	/** * 设置滚轮滚动监听 * * @param mItemSelectedListener */
	...
	/** * 设置滚轮是否为循环滚动 * * @param enable true-循环 false-单程 */
	...
	/** * 设置滚轮刻度透明渐变值 * * @param alphaGradual */
	...
	/**
	 * * 设置滚轮可显示的刻度数量，必须为奇数，且大于等于3 * * @param wheelSize * @throws
	 * CycleWheelViewException 滚轮数量错误
	 * */
	...
	/**
	 * * 设置块的颜色 * @param unselectedSolidColor 未选中的块的颜色 * @param
	 * selectedSolidColor 选中的块的颜色
	 */
	...
	/** * 设置分割线样式 * @param dividerColor 分割线颜色 * @param dividerHeight 分割线高度(px) */
	...
	private void initView() {
		...
		Drawable backgroud = new Drawable() {
			...
			public void draw(Canvas canvas) {
				...
				canvas.drawRect(0, 0, viewWidth,
						mItemHeight * (mWheelSize / 2), solidPaint);
				canvas.drawRect(0, mItemHeight * (mWheelSize / 2 + 1),
						viewWidth, mItemHeight * (mWheelSize), solidPaint);
				canvas.drawRect(0, mItemHeight * (mWheelSize / 2), viewWidth,
						mItemHeight * (mWheelSize / 2 + 1), seletedSolidPaint);
				canvas.drawLine(0, mItemHeight * (mWheelSize / 2), viewWidth,
						mItemHeight * (mWheelSize / 2), dividerPaint);
				canvas.drawLine(0, mItemHeight * (mWheelSize / 2 + 1),
						viewWidth, mItemHeight * (mWheelSize / 2 + 1),
						dividerPaint);
			}
	...
	public class CycleWheelViewAdapter extends BaseAdapter {
		...
		public View getView(int position, View convertView, ViewGroup parent) {
			...
			TextView textView = (TextView) convertView
					.findViewById(mItemLabelTvId);
			if (position < mWheelSize / 2
					|| (!cylceEnable && position >= mData.size() + mWheelSize
							/ 2)) {

				textView.setText("");
				convertView.setVisibility(View.INVISIBLE);
			} else {
				textView.setText(mData.get((position - mWheelSize / 2)
						% mData.size()));
